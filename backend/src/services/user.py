...
class UserService:
    ...
    async def register_user(self, user_data: UserCreate) -> UserResponse:
        ...
        async with self.uow:
            ...
            return UserResponse(
                ...
                role=user.role,
                token=str(user.token) if user.token else None,
            )

    async def login_user(self, user_data: UserLogin) -> TokenPair:
        # Поддерживаем вход по email или username
        if user_data.email:
        ...
        tokens = TokenPair(
            ...
            user_uuid=str(user.token) if user.token else None,
        )
        return tokens
    ...
    async def update_user(
        ...
        return UserResponse(
            ...
            role=updated.role,
            token=str(updated.token) if updated.token else None,
        )
